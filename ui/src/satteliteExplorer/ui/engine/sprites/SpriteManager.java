package satteliteExplorer.ui.engine.sprites;

import com.jme3.app.state.AbstractAppState;
import com.jme3.asset.AssetManager;
import com.jme3.scene.Node;
import satteliteExplorer.ui.engine.sprites.atlas.AtlasSprite;
import satteliteExplorer.ui.engine.sprites.atlas.SimpleTextureAtlas;
import satteliteExplorer.ui.engine.util.ImageUtilities;
...
public class SpriteManager extends AbstractAppState {
  ...
  private SpriteMesh.Strategy strategy;

  public SpriteManager(int maxAtlasWidth, int maxAtlasHeight, SpriteMesh.Strategy strategy, Node rootNode, AssetManager assetManager) {
  ...
  private SpriteMesh createSpriteMesh(boolean isStatic) {
    if (spriteMeshes.size() > 1)
      System.out.println("Warning: SpriteManager is forced to create an extra spriteMesh a) maxAtlasWidth=" + maxAtlasWidth + " maxAtlasHeight=" + maxAtlasHeight + " is small \nb) too many DIFFERENT atlasSprites. Total:" + images.size() + " c) total spriteMeshes=" + spriteMeshes.size() + ((strategy == SpriteMesh.Strategy.KEEP_BUFFER) ? " d) KEEP_BUFFER strategy is slow at this condition." : ""));
